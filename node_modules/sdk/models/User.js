...
class User {
  ...
  static cryptPassword(password) {
    // Legacy MD5 hashing (kept for backward compatibility)
    return md5(password);
  }

  static hashPassword(password) {
    // bcrypt synchronous hash with default salt rounds
    return bcrypt.hashSync(password, 10);
  }

  static verifyPassword(stored, plain) {
    if (!stored) return false;
    // If stored looks like a bcrypt hash (starts with $2), use bcrypt compare
    if (typeof stored === 'string' && stored.indexOf('$2') === 0) {
      try {
    ...
    // Fallback to legacy MD5 comparison
  ...
  set password(value) {
    if (this.self.password != value) {
      // Store password using bcrypt (new secure default)
      this.self.password = this.constructor.hashPassword(value);
    }
  }
