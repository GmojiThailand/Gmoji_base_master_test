...
router.all('/delivery_service',
  ...
  function* () {
    ...
    console.log("Delivery request", fields);
    ...
    if (this.user && this.user.role && this.user.role.id == RolesDictionary.USER) {
        const usersTable = yield Table.fetch('users', this.application.id);
        let user = yield usersTable.find({user_id: this.user.id})
            .catch((e) => { throw new HttpError(404, 'User not found'); });
        user = user.data;

        Object.assign(
            sendData,
            {
                userName: fields.user_name || fields.userName || user.name,
                userPhone: fields.user_phone || fields.userPhone || user.phone,
            }
        );
    } else {
