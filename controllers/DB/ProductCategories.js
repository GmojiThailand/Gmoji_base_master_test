/**
 * Запросы к пользовательской таблице product_categories
 */
'use strict';
...
router.get('/:data_id',
  ...
  function* () {
    ...
    let productCategory = yield productCategoriesTable.find(this.params.data_id, options);

    this.body = productCategory;
  });
...
router.get('/',
  ...
  function* () {
    ...
    const productCategoriesTable = yield Table.fetch('product_categories', this.application.id);
    let productCategories = yield productCategoriesTable.findAll(filter, options);

    this.body = productCategories;
  });
