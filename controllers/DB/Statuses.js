/**
 * Запросы к пользовательской таблице statuses
 */
'use strict';
...
router.get('/:data_id',
  ...
  function* () {
    ...
    const statusesTable = yield Table.fetch('statuses', this.application.id);

    let status = yield statusesTable.find(this.params.data_id, options);
...
router.get('/',
  ...
  function* () {
    ...
    const statusesTable = yield Table.fetch('statuses', this.application.id);
    let statuses = yield statusesTable.findAll(filter, options);
