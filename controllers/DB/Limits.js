/**
 * Запросы к пользовательской таблице limits
 */
'use strict';
...
router.get('/:data_id',
  ...
  function* () {
    ...
    let limit = yield limitsTable.find(this.params.data_id, options);
...
router.get('/',
  ...
  function* () {
    ...
    const limitsTable = yield Table.fetch('limits', this.application.id);
    let limits = yield limitsTable.findAll(filter, options);
